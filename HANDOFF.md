...
## 2026-09-24 听感练习答案工作台与指板重复音说明

- 听感练习主区域调整为明确的上下结构：上方是双栏工作台，左侧只保留当前调、题目 / 播放状态、生成新题、播放当前题和从头重听；右侧始终保留“旋律答案”卡片。下方“分级提示与自评”恢复为横跨整行的独立模块，包含逐级提示、自评、确认、下一题和困难题复习。
- 未揭晓答案时，右侧显示完整尺寸的答案占位卡片、虚线锁定区域和“核对旋律答案”按钮，避免初始页面右侧留空。揭晓后在同一位置显示简谱、节奏和推荐指板；收起时按钮固定返回虚线区域中央，不再错位到卡片底部。
- 当前没有有效题目时，点击“核对旋律答案”不会打开空白答案，而会提示先生成新题，或点击播放自动生成题目。生成失败且没有上一题时，答案卡片同步显示“尚未生成题目”。
- 推荐指板中同一弦品出现多个旋律序号，表示旋律重复使用同一个音，并非两个不同音映射到同一格。格内现在上方显示音高 / 简谱，下方显示“第 1·3 音”等出现顺序，悬停说明标记“同音重复”。
- 已运行全部八组自动化测试、`node --check green-ui/ear-app.js`、`git diff --check` 和统一构建。四份发布 HTML SHA-256 均为 `e844d0cea01a8c01ea951e6e171a44c2485fb4d080a241d73a87dfd2a8aed379`。
- 自动测试验证生成、音频时钟、任务状态和数据约束；真实扬声器、iPad 触控与最终视觉效果仍需用户在设备上验收。
